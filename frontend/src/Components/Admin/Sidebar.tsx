...
import { FaTachometerAlt, FaUser, FaRegListAlt, FaUserMd, FaBars, FaPlus, FaTimes } from 'react-icons/fa';
...
function Sidebar() {
    ...
    return (
        <>
            ...
            <div
                ref={sidebarRef}
                className={`hidden lg:block fixed top-14 left-0 h-[calc(100vh-3.5rem)] bg-white text-gray-800 transition-all duration-300 ${isExpanded ? 'w-64' : 'w-16'
                    } overflow-hidden shadow-lg border-r border-gray-200 z-10`}
            >
                ...
                <div className="mt-6">
                    <ul>
                        {menuItems.map((item, index) => (
                            <li key={index}>
                                <button
                                    className={`flex items-center p-4 w-full text-left transition-colors ${isActive(item.path)
            ...
            <div className={`lg:hidden fixed top-14 left-0 h-[calc(100vh-3.5rem)] w-64 bg-white text-gray-800 transition-transform duration-300 z-40 shadow-lg border-r border-gray-200 ${isExpanded ? 'translate-x-0' : '-translate-x-full'
                }`}>
                <div className="flex items-center justify-between p-4 border-b border-gray-200">
